...
DATA : lt_table_filds     TYPE TABLE OF dfies,
       ...
       lv_variable_1      TYPE char50,
       lv_variable_2(200) TYPE c,
...
*** --- Assign field symbol with Structure type of DDIC
  ...
  IF sy-subrc EQ 0.
    ...
    MOVE-CORRESPONDING lt_dd03l TO lt_tbl_fld.

    LOOP AT lt_tbl_fld ASSIGNING FIELD-SYMBOL(<lfs_tbl_fld>).
      <lfs_tbl_fld>-index = sy-tabix.
    ENDLOOP.

  ENDIF.

  DESCRIBE TABLE lt_tbl_fld LINES DATA(lv_lines).

  LOOP AT <ft_table> ASSIGNING <fs_table>.

    DO lv_lines TIMES.
      READ TABLE lt_tbl_fld INTO DATA(ls_fld) INDEX sy-index.
