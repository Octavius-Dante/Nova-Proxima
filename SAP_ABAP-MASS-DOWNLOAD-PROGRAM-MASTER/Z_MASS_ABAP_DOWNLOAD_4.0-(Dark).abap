...
* new inclusion - start
*----------------------------------------------------------------------------------------------------------------------
*  Table definition enhancement variables
*----------------------------------------------------------------------------------------------------------------------
...
DATA : lv_obj_name      TYPE sobj_name,
       lv_devclass_name TYPE devclass.
...
FORM checkclassdoesexist USING ilocclass LIKE iclasses[]
  ...
  SELECT clsname descript msg_id
         FROM vseoclass
         INTO CORRESPONDING FIELDS OF TABLE ilocclass
         WHERE clsname IN rangeclass
           AND version = '1'
           AND ( state = '0' OR state = '1' ).
...
*         FROM vseoclass
*         INTO CORRESPONDING FIELDS OF TABLE waclass
*         WHERE clsname IN rangeclass
*           AND version = '1'
*           AND ( state = '0' OR state = '1' ).
...
*    SELECT SINGLE clsname descript msg_id
*         FROM vseoclass
*         INTO CORRESPONDING FIELDS OF TABLE ilocclass "waclass
*         WHERE clsname IN rangeclass
*           AND version = '0'
*           AND ( state = '0' OR state = '1' ).
*  ENDIF.
...
* new inclusion - start
  SELECT obj_name devclass
    FROM tadir INTO (lv_obj_name ,lv_devclass_name) WHERE obj_name = tablename.
  ENDSELECT.
  IF sy-subrc EQ 0.
    CONCATENATE `  <h3>Package: ` lv_devclass_name `</h3></td>` INTO wahtml.
    APPEND wahtml TO ilochtml.
    CLEAR : wahtml, lv_devclass_name, lv_obj_name.
  ENDIF.
* new inclusion - end
  APPEND `    </tr>` TO ilochtml.
...
* new inclusion - start
  APPEND `<body bgcolor=#1C2228>` TO htmltable.
* new inclusion - end
...
* new inclusion - start
  SELECT obj_name devclass
    FROM tadir INTO (lv_obj_name ,lv_devclass_name) WHERE obj_name = programname.
  ENDSELECT.
  IF sy-subrc EQ 0.
    CONCATENATE `  <h3>Package: ` lv_devclass_name `</h3></td>` INTO htmltable.
    APPEND htmltable.
    CLEAR : htmltable, lv_devclass_name, lv_obj_name.
  ENDIF.
* new inclusion - end
...
* new inclusion - start
  SELECT obj_name devclass
    FROM tadir INTO (lv_obj_name ,lv_devclass_name) WHERE obj_name = classname.
  ENDSELECT.
  IF sy-subrc EQ 0.
    CONCATENATE `  <h3>Package: ` lv_devclass_name `</h3></td>` INTO htmltable.
    APPEND htmltable.
...
* new inclusion - start
  SELECT obj_name devclass
    FROM tadir INTO (lv_obj_name ,lv_devclass_name) WHERE obj_name = functionname.
  ENDSELECT.
  IF sy-subrc EQ 0.
    CONCATENATE `  <h3>Package: ` lv_devclass_name `</h3></td>` INTO htmltable.
    APPEND htmltable.
    CLEAR : htmltable, lv_obj_name ,lv_devclass_name.
  ENDIF.
* new inclusion - end
...
* new inclusion - end
  ...
  APPEND `   width:100%;` TO ilocheader.
* new inclusion - start
  APPEND `   border-color:#808080;` TO ilocheader.
* new inclusion - end
  APPEND `   border-top-width: thin;` TO ilocheader.
  APPEND `   border-right-width: thin;` TO ilocheader.
  APPEND `   border-bottom-width: thin;` TO ilocheader.
